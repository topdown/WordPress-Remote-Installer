...
switch ( $step ) {
	...
	case 0 :
		...
		if ( version_compare( WPRI_VERSION, $githubVersion = getGithubVersion(), '<' ) ) {
			?>
			<p class="version_alert">You are using Version <?php echo WPRI_VERSION; ?>. Version <?php echo $githubVersion; ?> is available through
				<a href="https://github.com/lucanos/WordPress-Remote-Installer">Github</a>.</p>
			<?php
		}
		?>
		<form method="post">
	...
	case 1 :
		...
		if ( ! $proceed ) {
			...
			<form method="post">
				<input type="hidden" name="step" value="1" /> <input type="hidden" name="action" value="cleanse" />
	...
	case 3 :
		...
		<form method="post">
			<textarea name="plugins"><?php echo $suggest; ?></textarea> <input type="hidden" name="step" value="4" />
		...
		<ul>
			...
			if ( isset( $_POST['plugins'] ) ) {
				...
				foreach ( $plugins as $url ) {
					...
					?>
					<li class="<?php echo( $plugin_result ? 'pass' : 'fail' ); ?>">Installing
						<strong><?php echo $bits[2]; ?></strong> - <?php echo $plugin_message; ?></li>
					<?php
				}
			}
			?>
	...
	case 5 :
		...
		<form method="post">
			<textarea name="themes"><?php echo $suggest; ?></textarea> <input type="hidden" name="step" value="6" />
			...
			if ( isset( $_POST['themes'] ) ) {
				...
				foreach ( $themes as $url ) {
					...
					if ( ! $get ) {
						...
						?>
						<li class="<?php echo( $theme_result ? 'pass' : 'fail' ); ?>">Installing
							<strong><?php echo $bits[2]; ?>.zip</strong> - <?php echo $theme_message; ?></li>
						<?php
...
<div id="footer">
	<a href="https://github.com/lucanos/WordPress-Remote-Installer" class="github">View on GitHub</a> Created by
	<a href="http://lucanos.com">Luke Stevenson</a><br />

	<div class="legal">
		<strong>NOTE:</strong> This script is not an official WordPress product.<br /> The WordPress logo is the property of the WordPress Foundation.
	</div>
</div>
